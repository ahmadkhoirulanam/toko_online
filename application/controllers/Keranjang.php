...
class Keranjang extends CI_Controller
{
    ...
    function index()
    {
        ...
        $warna   = filter($this->input->post('warna'));
		//$stok = $b['beli'] - $j['jual'];
		 
        $query = $this->db->get_where('produkvarian', array('id_produk' => $id_produk, 'warna_produkvarian' => $warna));
        foreach ($query->result() as $row) {
            $stok = $row->stok_produkvarian;
        ...
        if ($id_produk != '') {
            if ($stok < $this->input->post('jumlah') or $stok <= '0') {
                ...
                $cek = $this->model_app->view_where('penjualantemp', array('session' => $this->session->idp, 'id_produk' => $id_produk, 'warna' => $warna))->num_rows();
                if ($cek >= 1) {
                    $this->db->query("UPDATE penjualantemp SET jumlah=jumlah+$jumlah where session='" . $this->session->idp . "' AND id_produk='$id_produk' AND warna='$warna'");
                } else {
					
                    $harga = $this->model_app->view_where('produk', array('id_produk' => $id_produk))->row_array();
					
					//id_penjualan_detail	session	id_produk	warna	jumlah	harga_jual	satuan	waktu_order  
                    $data = array(
    ...
    function checkouts()
    {
        if (isset($_POST['submit'])) {
            if ($this->session->idp != '') {
                ...
                $usr = $this->db->query("SELECT email from pengguna where id_pengguna=$id_pengguna")->row_array();
            ...
            if ($this->session->id_pengguna) {
                ...
                if ($cek->num_rows() >= 1) {
					...
					$data['halamans'] = $this->model_halaman->halaman()->result();
                    $data['rows'] = $this->model_app->view_join_where_two('pengguna', 'alamat', 'kota', 'id_alamat', 'id_alamat', 'id_kota', 'kota_id', array('pengguna.id_pengguna' => $this->session->id_pengguna))->row_array();
