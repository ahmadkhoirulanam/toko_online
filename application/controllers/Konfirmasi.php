...
class Konfirmasi extends CI_Controller
{
	...
	function index()
	{
		if (isset($_GET['q'])) {
			...
			$data['rows'] = $this->model_app->view_where('penjualan', array('id_penjualan' => $row['id_penjualan']))->row_array();
	...
	function form()
	{
		...
		if (!$this->upload->do_upload('userfile')) {
			...
			$this->model_app->update('penjualan', $data1, $where);
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
             Terimakasih telah melakukan konfirmasi pembayaran.<br>
			Segera kami cek dan kami proses.
