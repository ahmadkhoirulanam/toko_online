...
class Produk extends CI_Controller
{
	...
	function kategori()
	{
		$cek = $this->model_app->edit('kategoriproduk', array('kategori_seo' => $this->uri->segment(3)))->row_array();
		$jumlah = $this->model_app->view_where('produk', array('id_kategori_produk' => $cek['id_kategori_produk']))->num_rows();
		...
		if (is_numeric($dari)) {
			...
			$data['namakategori'] = $cek[nama_kategori];
			$data['halamans'] = $this->model_halaman->halaman()->result();
			$data['kategoris'] = $this->model_kategori_produk->semua_kategori_produk(0,10)->result();
			
			$data['record'] = $this->model_app->view_where_ordering_limit('produk', array('id_kategori_produk' => $cek['id_kategori_produk']), 'id_produk', 'DESC', $dari, $config['per_page']);
	...
	function detail()
	{
		$query = $this->model_app->edit('produk', array('produk_seo' => $this->uri->segment(3)));
		...
		if ($query->num_rows() >= 1) {
			...
			$data['halamans'] = $this->model_halaman->halaman()->result();
			$data['kategoris'] = $this->model_kategori_produk->semua_kategori_produk(0,10)->result();
			$data['row'] = $this->model_app->view_where('produk', array('id_produk' => $cek['id_produk']))->row_array();
	...
	function keranjang()
	{
		...
		if ($id_produk != '') {
			if ($stok < $this->input->post('jumlah') or $stok <= '0') {
				$produk = $this->model_app->edit('produk', array('id_produk' => $id_produk))->row_array();
				...
				$cek = $this->model_app->view_where('penjualantemp', array('session' => $this->session->idp, 'id_produk' => $id_produk))->num_rows();
				if ($cek >= 1) {
					$this->db->query("UPDATE penjualantemp SET jumlah=jumlah+$jumlah where session='" . $this->session->idp . "' AND id_produk='$id_produk'");
				} else {
					...
					$data = array(
						...
						'id_produk' => $id_produk,
						'jumlah' => $jumlah,
						'harga_jual' => $harga['harga_ecer'],
						'satuan' => $harga['satuan'],
						'waktu_order' => date('Y-m-d H:i:s')
					);
					$this->model_app->insert('penjualantemp', $data);
				}
				redirect('produk/keranjang');
			}
		} else {
			$data['record'] = $this->model_app->view_join_rows('penjualantemp', 'produk', 'id_produk', array('session' => $this->session->idp), 'id_penjualan_detail', 'ASC');
	...
	function keranjang_update()
	{
		...
		if (!empty($id_produk)) {
			foreach ($id_pen as $index => $val) {
				...
				$this->db->update('penjualantemp', $data);
			}
		}
		redirect('produk/keranjang');
	}
	...
	function keranjang_update2()
	{
		...
		$this->db->update('penjualantemp', $data, "id_penjualan_detail='$id_pen'");
	...
	function keranjang_delete()
	{
		...
		$this->model_app->delete('penjualantemp', $id);
		$isi_keranjang = $this->db->query("SELECT sum(jumlah) as jumlah FROM penjualantemp where session='" . $this->session->idp . "'")->row_array();
	...
	function checkouts()
	{
		if (isset($_POST['submit'])) {
			if ($this->session->idp != '') {
				...
				$this->model_app->insert('penjualan', $data);
				$idp = $this->db->insert_id();

				$keranjang = $this->model_app->view_where('penjualantemp', array('session' => $this->session->idp));
				foreach ($keranjang->result_array() as $row) {
					...
					$this->model_app->insert('penjualandetail', $dataa);

					$q = $this->db->get_where('produk', array('id_produk' => $row['id_produk']));
					...
					$this->db->update('produk', $datastok);
				}
				$this->model_app->delete('penjualantemp', array('session' => $this->session->idp));
				//$kons = $this->model_app->view_join_where_one('tbl_toko_konsumen', 'kota', 'kota_id', array('id_pengguna' => $this->session->id_pengguna))->row_array();
				//$kons = $this->model_app->view_join_where_two('pengguna', 'alamat', 'kota', 'id_pengguna', 'id_pengguna', 'id_kota', 'kota_id', array('pengguna.id_pengguna' => $this->session->id_pengguna))->row_array();
				$kons = $this->db->query("SELECT * FROM penjualan a JOIN kota b ON a.p_kota=b.kota_id where a.id_penjualan='$idp'")->row_array();
				...
				$data['rekening'] = $this->model_app->view('rekening');
				...
				$no = 1;
				$belanjaan = $this->model_app->view_join_where('penjualandetail', 'produk', 'id_produk', array('id_penjualan' => $idp), 'id_penjualan_detail', 'ASC');
				...
				$message .= "</tbody>
				      ...
				      Salam. Admin BAKOLSMARTPHONE</body></html> \n";
			...
			if ($this->session->id_pengguna) {
				$cek = $this->model_app->view_where('penjualantemp', array('session' => $this->session->idp));
				if ($cek->num_rows() >= 1) {
					$data['title'] = 'Checkout';
					$data['breadcrumb'] = 'Checkout';
					$data['rows'] = $this->model_app->view_join_where_two('pengguna', 'alamat', 'kota', 'id_pengguna', 'id_pengguna', 'id_kota', 'kota_id', array('pengguna.id_pengguna' => $this->session->id_pengguna))->row_array();

					$ses = $this->session->idp;
					$this->db->join('produk', 'penjualantemp.id_produk=produk.id_produk');
					$this->db->where("penjualantemp.session='$ses'");
					$this->db->order_by('penjualantemp.id_penjualan_detail', 'ASC');
					$query = $this->db->get('penjualantemp');
	...
	function print_invoice()
	{
		...
		$data['record'] = $this->db->query("SELECT a.kode_transaksi, a.waktu_transaksi, b.*, c.nama_produk, c.satuan, c.berat, c.diskon FROM `penjualan` a JOIN penjualandetail b ON a.id_penjualan=b.id_penjualan JOIN produk c ON b.id_produk=c.id_produk where a.kode_transaksi='" . $this->uri->segment(3) . "'");
	...
	function review()
	{
		if (isset($_POST['submit'])) {
			...
			$cek = $this->db->query("SELECT * FROM `ulasan` where id_pembeli='$pembeli' AND id_produk='$produk'");
			if ($cek->num_rows() >= 1) {

				$this->db->query("UPDATE ulasan SET bintang='$bintang',ulasan='$ulasan' where id_pembeli='$pembeli' AND id_produk='$produk'");
