...
class Admin extends CI_Controller
{
	...
	function tambah_kategori_produk()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$this->model_app->insert('kategoriproduk', $data);
	...
	function edit_kategori_produk()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$this->model_app->update('kategoriproduk', $data, $where);
			redirect('admin/kategori_produk');
		} else {
	...
	function tambah_produk()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$data['record'] = $this->model_app->view_ordering('kategoriproduk', 'id_kategori_produk', 'DESC');
			$data['supp'] = $this->model_app->view_ordering('tbl_toko_supplier', 'id_supplier', 'DESC');
	...
	function tracking()
	{
		...
		if ($this->uri->segment(3) != '') {
			...
			$data['record'] = $this->db->query("SELECT a.kode_transaksi, b.*, c.nama_produk, c.satuan, c.berat,  c.produk_seo FROM `penjualan` a JOIN penjualandetail b ON a.id_penjualan=b.id_penjualan JOIN produk c ON b.id_produk=c.id_produk where a.kode_transaksi='" . $kode_transaksi . "'");
	...
	function supplier()
	{
		...
		$data['record'] = $this->model_app->view_ordering('tbl_toko_supplier', 'id_supplier', 'DESC');
	...
	function tambah_supplier()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$this->model_app->insert('tbl_toko_supplier', $data);
	...
	function edit_supplier()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$this->model_app->update('tbl_toko_supplier', $data, $where);
			redirect('admin/supplier');
		} else {

			$data['title'] = 'Edit Supplier';
			$proses = $this->model_app->edit('tbl_toko_supplier', array('id_supplier' => $id))->row_array();
	...
	function delete_supplier($id)
	{
		...
		$this->model_app->delete('tbl_toko_supplier', $where);
		echo json_encode(array("status" => TRUE));
	}
	...
	function delete_konsumen($id)
	{
		...
		$this->model_app->delete('tbl_toko_konsumen', $where);
		echo json_encode(array("status" => TRUE));
	}
	...
	function edit_user()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$this->model_app->update('pengguna', $data, $where);
			...
			$proses = $this->model_app->edit('pengguna', array('username' => $id))->row_array();
			$data = array('rows' => $proses);
	...
	function users()
	{
		...
		// $data['record'] = $this->model_app->view_ordering('pengguna', 'username', 'DESC');
		$data['record'] = $this->db->query("SELECT * FROM pengguna ORDER BY level ASC,username ASC")->result_array();
	...
	function tambah_newsletter()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$ci->email->from('demo-ecommerce@zamanet.com', "BAKOLSMARTPHONE");
