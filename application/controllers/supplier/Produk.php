...
class Produk extends CI_Controller
{
	...
	function index()
	{
		...
		$data['title'] = 'Kelola Produk Supplier'; 
		$data['record'] = $this->model_app->view_ordering('produk', 'nama_produk', 'ACS');
		$this->template->load('supplier/template', 'supplier/produk/view_produk', $data);
	}
	...
	
	{
		...
		if (isset($_POST['submit'])) {
			...
			$this->template->load('supplier/template', 'supplier/produk/view_produk_tambah', $data);
		}
	...
	function edit_produk()
	{
		...
		if (isset($_POST['submit'])) {
			...
			if($update) {
				$this->session->set_userdata('Sukses', 'Data produk supplier berhasil diupdate.' ); 
				redirect('supplier/produk');
			} else {
				$this->session->set_userdata('Gagal', 'Data produk supplier gagal diupdate.' ); 
				redirect('supplier/produk');
			}
			...
			$this->template->load('supplier/template', 'supplier/produk/view_produk_edit', $data);
		}
	}
	...
	function kelola_varian()
	{
		...
		$idproduk = $this->uri->segment(4);
			...
			$this->template->load('supplier/template', 'supplier/produk/view_kelola_varian', $data);
	...
	function tambah_varian()
	{
		...
		if($simpan) {
			...
			redirect('supplier/produk/kelola_varian/'.$this->input->post('idproduk'));
		} else {
			...
			$this->load->view('supplier/produk/view_varian_tambah', $data);
		}
	}
	...
	function edit_varian()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$this->load->view('supplier/produk/view_varian_edit', $data);
		}
	}
