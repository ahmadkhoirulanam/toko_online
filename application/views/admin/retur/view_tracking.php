  ...
  <div class="row bg-title">
		  ...
		  <div class="row">
			...
			<h3 class="box-title m-b-0">Detail Retur </h3><br/>
		   


              <?php
              if ($totalretur['status_retur'] == 'Baru') {
                $prosesretur = '<span class="text-danger">Baru</span>';
                $colorretur = 'danger';
                $textretur = 'Baru';
              } elseif ($totalretur['status_retur'] == 'Proses') {
                $prosesretur = '<span class="text-warning">Proses</span>';
                $colorretur = 'warning';
                $textretur = 'Proses';
              } elseif ($totalretur['status_retur'] == 'Selesai') {
                $prosesretur = '<span class="text-primary">Selesai</span>';
                $colorretur = 'primary';
                $textretur = 'Selesai';
              } 
              ...
              ?>
                ...
                <div class='btn-group' >
                  <button style='width:100px' type='button' class='btn btn-<?= $colorretur ?> btn-sm'  ><?= $textretur ?></button>

                  <button type='button' class='btn btn-<?= $colorretur ?> btn-sm dropdown-toggle' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'  > <span class='caret'></span> <span class='sr-only'>Toggle Dropdown</span> </button>
                  <div class='dropdown-menu' style='border:1px solid #ccc !important;'>
                    <a class='dropdown-item' href='<?= base_url('admin/retur/retur_status2/') . $totalretur['id_retur'] ?>/Baru/<?= $this->uri->segment(4) ?>' onclick="return confirm('Apa anda yakin untuk ubah status jadi Baru ?')"> Baru</a>
                    
					<a class='dropdown-item' href='<?= base_url('admin/retur/retur_status2/') . $totalretur['id_retur'] ?>/Proses/<?= $this->uri->segment(4) ?>' onclick="return confirm('Apa anda yakin untuk ubah status jadi Proses ?')"> Proses</a>
                    <a class='dropdown-item' href='<?= base_url('admin/retur/retur_status2/') . $totalretur['id_retur'] ?>/Selesai/<?= $this->uri->segment(4) ?>' onclick="return confirm('Apa anda yakin untuk ubah status jadi Selesai ?')"> Selesai</a>
                
                  </div>
                </div>

                <a class='btn btn-success btn-sm' href='<?php echo base_url('admin/retur'); ?>'>Kembali</a>
              <br/><br/>
			  
		<div class="col-md-6">
                  <table class="table table-sm table-borderless" style="width: 100%">
                    <tr>
                      <td style="width:120px;">Tanggal Retur</td>
                      <td><?= $totalretur['waktu_transaksi']; ?></td>
                    </tr>
                    <tr>
                      <td>Video Unboxing</td>
                      <td><a href="<?= base_url() ?>/assets/images/bukti/<?= $totalretur['video_unboxing']; ?>" target="_blank"><?= $totalretur['video_unboxing']; ?></a></td>
                    </tr>
                    ...
                    <tbody>
                      ...
                      ?>
                        ...
                        <tr>
                          ...
                          <td><a href='<?= base_url('produk/detail/') . $row['produk_seo'] ?>' target='_blank'><?= $row['nama_produk']; ?></a></td>
